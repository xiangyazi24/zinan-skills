...
When a single `sorry` covers multiple independent cases (e.g., one sorry after a case-split that handles N constructors), replace it with N per-case lemma stubs, each with the CORRECT type signature and a `sorry` body. The main theorem becomes a combiner that dispatches to each stub via `exact`. This lets you: (1) verify the combiner architecture type-checks immediately, (2) attack each stub independently (even in parallel), (3) land partial progress as each stub closes.
**Why:** A monolithic sorry after a case-split hid 8 independent sub-problems with different tactic needs (some needed extra `cases k₁ <;> cases k₂`, some needed `Cont'` case analysis, one needed no extra cases). Decomposing into 8 typed stubs made each independently buildable — the combiner verified immediately, and each stub can be filled by a Codex/subagent task without touching the others.
**How to apply:** When a sorry sits after a case-split (or could be decomposed into independent sub-goals), write per-case lemmas with the correct type extracted from the goal state, each body `sorry`. Wire the combiner with `exact`. Build to verify the architecture type-checks. Then fill each stub independently — this parallelizes and de-risks the work.
...
A shear substitution (e.g., `q → q + c·m`) cleanly reindexes a summable family `Σ_{r,t} f(r,t)` ONLY if the sheared coordinate of each term depends on the surviving summation index `r` but NOT on the inner index `t`. When two families in the same project have different coordinate structures — one with `m = g(r)` (t-independent, so the shear gives a clean r-dependent shift) and another with `m = h(r,t)` (t-dependent, so the shear gives an (r,t)-dependent shift) — the same shear produces a clean sigma/tau relation for the first family's hsum but NOT the second's. The failure is mathematical, not a coding issue: no single-variable shear + constant correction can simultaneously absorb the summation-index-dependent part AND match a target shift.
**Why:** A sigma shear (`q → q + 45m`) cleanly reindexed one row family (whose m-coordinate = -2r, t-independent) but failed for a symmetric row family (whose m-coordinate = 2r - t, t-dependent). Hours were spent trying to build a bridge before computing the shear's action on one representative term and discovering the t-dependence. An independent verifier confirmed the obstruction. The fix: use a DIFFERENT shear (tau, acting on s instead of m) for the second family — or bypass the shear-based proof entirely.
**How to apply:** Before building shear-based reindexing infrastructure for a summable family, compute the shear's action on ONE representative term: does the shift depend ONLY on the outer summation index, or also on the inner/branch index? If the latter, the shear does not produce a clean hsum-level relation — look for a different shear (acting on the coordinate that IS index-independent) or a shear-free proof route. The diagnostic is a one-line exponent computation, not a multi-file build.
...
Before declaring a sorry "hard" or "needs new theory," run `grep -c sorry` on ALL infrastructure files the sorry depends on. If every infrastructure file has zero sorry, the gap is ASSEMBLY (connecting proved components), not missing theory — categorically easier and more predictable. Conversely, if infrastructure files have their own sorries, those cascade and the real difficulty is upstream. This audit takes 30 seconds and changes the difficulty estimate by an order of magnitude, preventing both over-estimation (building unnecessary new theory when assembly suffices) and under-estimation (attempting assembly when the real gap is upstream).
**Why:** Three sorries were initially classified as "hard" (one labeled "crux," two labeled "mechanical engine"). A 30-second `grep -c sorry` across 7 infrastructure files showed ALL were zero-sorry. The "hard" sorries were pure assembly of proven components — connecting existing coefficient normal forms to an existing telescope identity. The difficulty dropped from "weeks of new mathematics" to "days of wiring." The same audit also revealed an import-chain issue (the sorry's file couldn't access the infrastructure due to circular imports), redirecting effort to file restructuring rather than new proofs.
**How to apply:** When facing a sorry or a set of sorries, before planning attack: `grep -c sorry InfraFile1.lean InfraFile2.lean ...` for every file in the sorry's transitive import closure. All-zero = assembly task (connect existing pieces). Any nonzero = trace the upstream sorry chain first; the real gap is there.

### [2026-06-28] `set x := expr/k` makes `k*x` opaque to `rw`/`exact` — use `conv` or avoid `set` for divided expressions
When `set M := expr / k` introduces a local definition, `k * M` unfolds to `k * (expr/k)` which is NOT definitionally equal to `expr` (integer division has no definitional inverse). This breaks `rw [hM_val]` (pattern `k*M` not found since it's unfolded), `exact h` (type mismatch), `Eq.trans` (definitional inequality), and `simp + exact`. The fix: (a) use `conv_lhs => rw [show k*M = expr from hM_val]` which gives `rw` precise scope, or (b) avoid `set` entirely for integer-divided expressions — use `have` + explicit divisibility proofs instead.
**Why:** Spent 15+ build iterations fighting this one step (`(4M)*(4N) = 5*(4k)^4` from `hUV`) — every standard tactic failed due to `set`-induced definitional unfolding of integer division.
**How to apply:** Any proof involving `set x := expr / k` where you later need equalities about `k * x`. Prefer `have hx : k * x = expr := Int.mul_ediv_cancel' hdvd` as a standalone fact and use `conv` for targeted rewriting.

### [2026-06-28] An existential bound `∃ C, f(N) ≤ C*g(N)/N` is VACUOUS for convergence if C grows with N — audit the witness
When a formalization provides `∃ C > 0, f(N) ≤ C * T / N` (meant to show O(1/N) decay), the existential is technically satisfiable with `C = large(N)`, giving `C*T/N ≈ const` — the bound is satisfied but USELESS for showing `f(N) → 0`. This is a DIFFERENT vacuity from unsatisfiable hypotheses (§3.3): the statement IS true, the structure DOES instantiate, `#print axioms` is clean, but the mathematical conclusion (convergence) does not follow. The witness C must be INDEPENDENT of N for the bound to deliver convergence.
**Why:** A frozen martingale QV bound gave `∃ C > 0, E[sup M²] ≤ C*T/N` with `C = K*N/T + 1` (from a crude norm bound on ‖M‖). The product `C*T/N = K + T/N ≈ K` (constant), so Markov gives `μ{M² ≥ δ²} ≤ K/δ²` (constant, NOT → 0). The bound was technically correct but useless for Kurtz convergence. The fix: prove a UNIFORM QV bound where C is independent of N (via stopped-martingale Doob + instantaneous QV rates ≤ C₀/N, giving uniform C₀).
**How to apply:** When a convergence proof depends on an existential bound `∃ C, integral ≤ C * T / N`, audit the WITNESS: does C depend on N? If `C = f(N)` with f growing, the bound is O(1), not O(1/N), and the convergence argument breaks. The fix: find a UNIFORM witness using finer structural information (rates, jumps, stopped processes) instead of crude norm bounds. Pairs with §3.3 vacuity entries (those catch unsatisfiable hypotheses; this catches vacuous-by-growth-of-witness existential bounds).

### [2026-06-28] BddAbove on a noncompact domain via left-zero / compact-mid / explicit-tail
When a continuous function f : ℝⁿ → ℝ vanishes on a half-space (e.g., cutoff kills t < c) and you need `BddAbove (Set.range f)`, split into three regions: (1) LEFT: f = 0 via `Filter.EventuallyEq` + `iteratedFDeriv_const`, (2) MID: bounded on a compact strip via `isCompact_Icc.exists_bound_of_continuousOn`, (3) TAIL: explicit bound from the analytic structure (L∞ contraction, eigenvalue damping, etc.). Assemble with a generic `bddAbove_range_of_left_mid_tail` wrapper. The compact-mid region absorbs the transition zone; the tail only needs the "clean" region where the cutoff is constant.
**Why:** Attempted `Continuous.bddAbove_range` (false on noncompact domains) and `HasCompactSupport` (false when the function is nonzero at ∞). The left/mid/tail split is the ONLY correct shape for cutoff-modified functions on unbounded domains.
**How to apply:** Any `BddAbove (Set.range ‖iteratedFDeriv ...‖)` where the function involves a smooth cutoff that kills one end but is constant (not zero) at the other end.

### [2026-06-28] `ContinuousOn.congr` takes `Set.EqOn g f s` — the argument is REVERSED from `f = g`
`ContinuousOn.congr (hf : ContinuousOn f s) (h : Set.EqOn g f s) : ContinuousOn g s` — the equality argument `h` must show the NEW function `g` equals the OLD function `f` on `s`, not the other way around. Three commits were burned on "Application type mismatch" from passing `∀ x ∈ s, f x = g x` when the API expects `∀ x ∈ s, g x = f x`.
**Why:** The natural mathematical phrasing "f agrees with g, so g is also continuous" writes the equality as f = g, but `Set.EqOn` in Mathlib is defined as `∀ x ∈ s, f x = g x` where f is the FIRST argument — and `ContinuousOn.congr` puts the NEW function first in the EqOn.
**How to apply:** Any use of `ContinuousOn.congr` or `ContinuousOn.congr_mono`. Write the `Set.EqOn` as `fun x hx => show NEW_FUNC x = OLD_FUNC x from ...`.

### [2026-06-28] A `classical`-defined function is opaque to simp even on concrete arguments — use proven reduction lemmas instead
A function defined as `by classical; exact if ∃ a, ... then X else ...` wraps every `if` through `Classical.dec`, which is an axiom the kernel won't reduce. So `simp [theFunction]` unfolds the definition but then gets STUCK at `dite (∃ a, ...) (Classical.dec _) ...` — even when both arguments are concrete lists where the existential is trivially true or false. The fix: DON'T put the function in simp. Instead, prove a small API of reduction lemmas (`f_case1 : f X Y = result1`, `f_case2 : ...`) that pattern-match on the concrete argument shapes, and put THOSE in simp. Verify one subgoal closes with the reduction API before scaling to the full proof.
**Why:** A case-analysis proof with ~200 subgoals failed on every one because the simp set unfolded a `classical`-defined classifier to `dite` terms that couldn't reduce further. Replacing the definition with 4 proven reduction lemmas (self/push/pop/replacement) closed all subgoals.
**How to apply:** When `simp [f]` leaves goals containing `dite ... (Classical.dec _) ...` or `Classical.choice` terms — the function `f` is classical-opaque. Switch to an API of reduction lemmas. Also check for a "replacement" pattern (e.g., `a :: L → b :: L`) that doesn't match push/pop/self.
...
When simp leaves goals like `(if c then (a,b) else (d,e)).2.length = ...` — projections, `List.length`, and other functions applied to an if-then-else — add Mathlib's `apply_ite` and `ite_apply` to the simp set. `apply_ite` says `f (if P then x else y) = if P then f x else f y` for any function f, pushing ALL function applications inside the if. For constructors with ≤2 nested ifs (e.g., clear, copy), adding `apply_ite, ite_apply` directly to `simp [...]` works. For constructors with 3+ nested ifs (e.g., succ with 3, pred with 4), `apply_ite` in the initial `simp` hits maxRecDepth — simp tries to distribute through every nesting level recursively. Fix with two phases: `simp only [defn-unfolding lemmas, WITHOUT apply_ite]` to expose the nested ifs, then `repeat split` to manually split ALL ifs into separate goals, then `simp_all [..., apply_ite, ite_apply]` on each (now shallow) branch.
**Why:** A 9-constructor proof had 7/9 proofs stuck on `(if ... then tuple_a else tuple_b).proj.length` goals. Adding `apply_ite` fixed the 2-level-nested cases immediately, but hit maxRecDepth on the 3-4 level cases. The two-phase approach (simp only → repeat split → simp_all with apply_ite) closed all 9 constructors.
**How to apply:** When simp leaves goals with function applications wrapping an unreduced if-then-else. Check the nesting depth first: ≤2 levels → `apply_ite` in main simp; ≥3 levels → two-phase approach.

### [2026-06-28] Numerically verify LLM-proposed proof intermediates BEFORE building infrastructure on them
When a collaborator (ChatGPT, Codex, or another LLM) proposes a specific mathematical identity as a proof intermediate — a coefficient decomposition, a bridge identity, a cancellation formula — verify it with a 10-line exact-arithmetic Python check BEFORE writing Lean infrastructure around it. LLM-proposed intermediates fail at a much higher rate than your own conjectures (they hallucinate plausible-sounding identities from pattern-matching on similar formulations). A single numerical counterexample kills the route instantly; building infrastructure first wastes hours on a dead claim.
**Why:** Three consecutive ChatGPT-proposed proof intermediates in one session were numerically FALSE: a coefficient bridge identity (6+ mismatches), a diagonal decomposition formula (sum=2 instead of 0 at the base point), and individual telescope lemmas (Hecke q-step nonzero while endpointTheta=0). Each was caught by a sub-minute Python computation, but only after time was spent analyzing or coding the proposed route. The falsified claims were all structurally plausible (correct types, right shape) — only numerical evaluation distinguishes them from the true identity.
**How to apply:** Any time an LLM proposes "the proof goes through intermediate identity X": write `python3 -c "..."` to evaluate BOTH sides of X at 3-5 concrete parameter values BEFORE writing any Lean code or analysis around X. If the values mismatch at even one point, the identity is false — reject and re-query. This extends "counterexample-test your own lemmas" to COLLABORATOR-proposed lemmas, where the false-positive rate is higher.
...
When a tactic proof contains `exact theorem_name arg1 ... (by sorry) argK ... argN`, the `sorry` does not just leave one sub-goal open — it makes the ENTIRE elaboration of `theorem_name` succeed trivially, so `argK+1` through `argN` are NEVER type-checked. Removing the sorry can expose errors in the LATER arguments (wrong argument count, type mismatch, non-function applied to an argument) that were completely hidden. A "build succeeded with sorry" does NOT mean the non-sorry parts compiled correctly.
**Why:** An `exact gronwall_theorem ... (by sorry) hsup hsup` call compiled with sorry. The second `hsup` was wrong (applied to a non-function type), but sorry suppressed the check. Filling in the sorry proof exposed the error, leading to hours of debugging a "regression" that was actually a pre-existing bug. The fix (removing one argument + restructuring) was simple once identified but hard to find because the error messages pointed to the PROOF CONTENT, not the argument count.
**How to apply:** After filling a sorry inside an `exact` call: if the build fails, check whether the ARGUMENT LIST (not just the proof) is correct. Count the expected vs actual arguments. A sorry-passing build guarantees NOTHING about arguments that follow the sorry'd one.

### [2026-06-28] `let` bindings in a theorem's argument type conflict with `set` variables — use `show` to force the outer name
When a theorem's argument type contains `let x := e; body`, and your proof context has `set x := e'` (where `e` and `e'` are definitionally equal but syntactically different), `apply`/`rw` with lemmas referencing `x` will fail: the goal's `x` is from the theorem's `let`, while your `x` is from the outer `set`. Lean's unifier won't match them through the `let`/`set` indirection. Fix: insert `show <goal restated with the outer set variable>` before the `apply`/`rw` to force the goal into the form that matches your proof context.
**Why:** A Gronwall theorem had `let δ := ε/(2*C)` in its h_integral_ineq type. The outer proof had `set δ := ε/(2*C_exp)` (definitionally equal). `apply (pi_norm_le_iff_of_nonneg ...).mpr` failed because the goal's `δ` (from `let`) didn't unify with the proof's `δ` (from `set`). Adding `show <type with outer δ>` resolved it immediately.
**How to apply:** When `apply`/`rw` fails with "could not unify" inside a `by` block that proves an argument with `let` bindings in its type: check if the goal contains unreduced `let` variables that shadow your `set` variables. If so, add `show <explicit goal>` using the `set` names before the failing tactic.

### [2026-06-28] Bound a tsum via `abs_tsum ≤ tsum_abs ≤ tsum_majorant` — write the helper once
When you need `|∑' n, f n| ≤ C`, the canonical Lean chain is: `norm_tsum_le_tsum_norm` (needs `Summable (‖f‖)` — use `hf.norm`, NOT `hf`) to get `≤ ∑' ‖f n‖`, then `Summable.tsum_le_tsum` (the METHOD form, arg order: `hfabs.tsum_le_tsum hfg hg`) to compare with a summable majorant `g`. Package this into a reusable `abs_tsum_le_tsum_of_abs_le` helper that takes `(hfg : ∀ n, |f n| ≤ g n) (hg : Summable g)` and returns `|∑' f| ≤ ∑' g`. The per-term bound typically uses `abs_mul` + `abs_neg` + domain-specific pointwise estimates; the summability of the majorant comes from the domain's spectral/eigenvalue summability theorem.
**Why:** Spent multiple commits debugging `norm_tsum_le_tsum_norm` (wrong summability argument: `Summable f` vs `Summable ‖f‖`), `tsum_le_tsum` (standalone doesn't exist, it's `Summable.tsum_le_tsum`), and `abs_tsum_le_tsum_abs` (doesn't exist in Mathlib). Writing the helper once eliminated all three failure modes.
**How to apply:** Any goal `|∑' n, f n| ≤ C` where you have term-by-term bounds and a summable majorant. Write the helper FIRST, then instantiate with the specific per-term estimate.

### [2026-06-28] Zero-radius placeholders in a multi-field bundle skeleton must be SATISFIABLE per field — not just per type
When constructing a large record with sorry stubs, setting all radius/bound parameters to 0 trivializes nonnegativity/convergence/boundedness fields. But a field like `|f x - g x| ≤ radius` with `radius = 0` forces `f x = g x` — which is a strictly STRONGER claim than you intend. Check each field: can the placeholder value make the field statement TRUE, not just well-typed?
**Why:** A skeleton set `Rspread = 0` to trivialize `0 ≤ Rspread`. But the spread field `|branch_v(u) - branch_w(u)| ≤ Rspread` became `|...| ≤ 0`, forcing all branches to agree — an unsatisfiable constraint. Changing to `Rspread = 2 * Rbr` (a carried positive bound) fixed it.
**How to apply:** When choosing placeholder values for a sorry skeleton, for each field: read the CONCLUSION type, substitute the placeholder, and ask "is this statement satisfiable?" A placeholder that makes one field trivially true but another field vacuously false is worse than sorry.

### [2026-06-28] Lift a blocking sorry to a constructor PARAMETER to unlock downstream sorry
When a sorry S blocks N downstream sorry (because they need S's value), and S requires genuine new mathematical work: add S as an explicit hypothesis to the constructor function. This closes S (it's now a parameter), closes the N downstream sorry (they wire through it), and makes the obligation visible at the call site. The parameter IS the sorry — promoted from an internal stub to an interface contract.
**Why:** A skeleton had `hqL := sorry` (winner dominance, needs new barrier theorem). Three other fields (hgap, hRa, hloser) depended on it. Lifting hqL to a constructor parameter closed all four sorry at once: hqL = parameter, hgap = `existing_theorem herr hutube_win`, hRa = `existing_bound hqL_at_start`. The total sorry count dropped by 3 with zero new proofs.
**How to apply:** When a sorry is both (a) genuinely hard (needs new math) and (b) a gateway for ≥2 downstream sorry: promote it to a constructor parameter instead of leaving it as sorry. The downstream closures are free wiring. This is the "conditional construction" pattern: prove the construction conditional on the hard obligation, then close the obligation separately.

### [2026-06-28] A documented "numerically FALSE" blocking claim may itself be false — independently re-verify before trusting
When a previous session's numerical script declares a key identity false and blocks a proof route, treat the claim as a hypothesis, not ground truth. Run an independent exact-integer verification with ALL contributing terms (all rows, all branch cases, correct sign conventions). A single missed term or wrong sign convention in the original script can produce false counterexamples.
**Why:** A numerical script missed contributions from one row at non-lexPositive denominators, producing four specific "counterexamples" that blocked a proof route for multiple sessions. Independent verification showed all four were wrong — the identity was true all along.
**How to apply:** When you encounter a doc or comment saying "THIS STATEMENT IS FALSE — numerics prove it", check whether the numerical script covered all cases (especially branch sign conventions and parity edge cases). A 10-minute re-verification can unblock days of stalled work.
...
When two finite Hahn-series sums have the same total at each q-degree but different individual term contributions, a pointwise bijection between their index spaces does NOT exist. Instead, look for an involution of the q-degree polynomial (a substitution that preserves the degree) and check whether the sign function has a clean pairing identity under that involution. The proof is a sum-level telescope, not a term-by-term correspondence.
**Why:** Spent hours trying to find a (row-index, branch-index) → (Hecke-u, Hecke-v) bijection that matched both q-degrees AND coefficients. The q-degree maps were correct (proved by ring), but the signs mismatched pointwise. The identity held only because an involution U → 1-U-V paired terms at the same q-degree with compensating signs.
**How to apply:** When a numerical check confirms an identity but `finsum_congr` on individual terms fails, test the q-degree polynomial for involutions (ring-verified). If one exists, prove the sign pairing under the involution (often by `split_ifs <;> omega`). The proof structure is: involution preserves q-degree + sign pairing under involution → sum-level equality.
