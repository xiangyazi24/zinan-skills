...
```bash
# 就这一种用法，没有参数：
python3 ~/.openclaw/workspace/scripts/ask-gpt.py "your question"

# 长问题用 stdin:
python3 ~/.openclaw/workspace/scripts/ask-gpt.py <<'EOQ'
your long, multi-line question…
EOQ
```

**不用指定 channel。** 脚本自动检测你的 tmux 窗口 → 查 bridge API →
找空闲 channel → 发出去。channel 管理完全机械化，你不用管。

窗口隔离也是自动的——脚本只会选本窗口 group 里的 channel，不会串台。
...
When you're stuck or grinding hard on a sub-problem, the bottleneck is frequently not the execution but the FRAMING: you're solving the wrong problem, or treating a legitimate event as a rare failure, or looking for a concentration bound on something that is actually deterministic / structural. Asking the collaborator "is X the right object / is this danger actually rare / what mechanism controls Y" produces structural re-framings that dissolve the grind entirely — whereas "prove X" or "bound Y" grinds within the (possibly wrong) framing. The design-level question is both cheaper to ask AND higher-value to receive.
**Why:** A sub-problem was framed as "prove this danger event has exponentially small mass" (a concentration tail); the collaborator's design-level answer ("the danger IS the legitimate transition gate — your framing is a category error; the right object is a convergence window, not a tail") dissolved the entire grind into a structural insight + a trivial closure. The same framing pattern appeared in three other sub-problems in the same session, each resolved by a "what's the right object" question.
**How to apply:** Before dispatching "prove/bound/compute X," ask yourself: am I confident X is the RIGHT object? If the sub-problem resists several attack vectors, dispatch a DESIGN question first: "is X the right quantity / is this event actually rare / what mechanism controls this / is my framing correct?" The collaborator's structural re-framing is worth more than its proof of the wrong thing. Pairs with "roadmap not proof" and "split tactical vs strategic across channels."
...
The highest-value collaborator dispatch in a deep assembly campaign is NOT "write the next producer" or "check if X is true" — it is a FEASIBILITY AUDIT: "here are the constraints my wiring imposes; are they simultaneously satisfiable for the concrete parameters?" You, the builder, are blind to structural impossibilities in your own wiring because you chose each piece to locally fit; the collaborator reads the global constraint set fresh and can spot a corner case (e.g. a denominator that vanishes, a parameter-sharing that creates `positive ≤ something × 0`) that your incremental wiring never exercised.
**Why:** I wired a multi-field assembler that tied two logically distinct parameters to the same value (a type-level shortcut). A feasibility-audit dispatch to the collaborator read the resulting budget constraint set, checked the corner `current_error = 0`, and flagged the inequality as unsatisfiable — a structural impossibility I had never checked despite building the assembler myself, and that no leaf-discharger could have surfaced.
**How to apply:** After wiring a multi-constraint assembler or record, before grinding the leaf-discharges, dispatch a "feasibility audit" question: give the collaborator the full set of schedule constraints the wiring imposes (the hypothesis list) + the concrete parameter values, and ask "are these simultaneously satisfiable? check corners." This catches wiring-level impossibilities early — before you grind many leaves that will be wasted on an unsatisfiable system. Complements two-way-audit (which catches per-result overclaims) and assemble-early (which catches missing producers) with CONSTRAINT-SET-LEVEL satisfiability.
...
When the task is "read N existing files' signatures, write a new file that threads them, build-verify" — a local sub-agent (with direct filesystem read/write + SSH build) completes faster than a ChatGPT channel (which reads through the GitHub connector, often misses files, and can't build). Reserve ChatGPT for DESIGN questions (feasibility audits, architecture analysis, "is X possible") and HARD REASONING (deep math analysis, counterexample search). Use sub-agents for IMPLEMENTATION (write code, build, fix errors, verify axioms).
**Why:** A sub-agent produced + built + verified a 2-theorem shifted-Euclidean assembly file while the parallel ChatGPT channel was still processing the same question (7+ minutes into its long-think). The sub-agent read the signatures directly from disk, wrote the Lean code, scp'd to the build server, fixed errors, and committed — all faster than the round-trip to ChatGPT.
**How to apply:** When dispatching: if the task is "read signatures + write Lean code + build", use a sub-agent (direct access, fast iteration). If the task is "should we do X? / what does Y imply? / is Z satisfiable?", use ChatGPT (strong reasoning, repo-connected for reading). The dispatch-selection criterion: reasoning-heavy → ChatGPT; implementation-heavy → sub-agent.
...
When dispatching a "which route should we take?" strategic question to a collaborator on a long-running project, include the project's documented terminal verdicts on previously-attempted routes (the run log's "all X approaches REFUTED" conclusions, the specific obstruction for each). Without this context, the collaborator's recommendation is made from a fresh read of the code alone — and it will naturally gravitate toward the route that LOOKS shortest from the code structure, even if that route was exhaustively tried and declared dead in prior sessions. The collaborator reads the repo; it does NOT read your run logs or prior-session handoff notes.
**Why:** A strategic route-assessment question omitted the project's 14-session terminal verdict that all direct algebraic routes were dead (hitting an irreducible 2D-coupled reflection core). The collaborator read the code, saw the direct route had the most infrastructure, and recommended it as "shortest" — exactly the route that had been exhaustively failed. Including "routes A/B/C were tried in N sessions and hit obstruction X/Y/Z" would have focused the answer on genuinely untried alternatives.
**How to apply:** When asking "which route?" on a project with prior failed attempts: (1) list each previously-tried route + its specific terminal obstruction in the prompt; (2) explicitly say "these are DEAD — I need a route that avoids obstruction X." The collaborator's comparative advantage is reasoning about UNTRIED routes, not re-discovering that a tried route looks plausible from the code. Complements "pre-verify premises you embed" (that's about false inclusions; this is about critical omissions) and "split tactical vs strategic" (this constrains the STRATEGIC channel's prompt).

### [2026-06-25] CAS-verify with SYMBOLIC indeterminates to discover whether a proof needs quotient-ring machinery
Before implementing a formal proof of a polynomial identity (in a quotient ring, coordinate ring, or directly), dispatch a CAS verification with the constituent polynomials as FORMAL SYMBOLS (not expanded into their explicit coefficient forms). The CAS result reveals whether the identity holds as an EXACT polynomial equality (all coefficients zero) or only modulo a relation (nonzero remainder). This is a decisive gate: exact equality → direct `ring`/`linear_combination` proof with no quotient infrastructure; modulo-relation → needs `AdjoinRoot`/coordinate-ring `mk` machinery. Discovering this BEFORE writing Lean saves choosing the wrong proof architecture.
**Why:** A polynomial identity was assumed to need coordinate-ring quotient machinery (AdjoinRoot, 50-100 line proof). A CAS test with formal symbol substitution revealed the quotient-ring coefficient was 0 — the identity was exact, needing only `linear_combination` with 4 hypotheses + `ring`. This completely changed the implementation from a complex quotient-ring proof to a short algebraic one. Had the CAS test been run first, the wasted AdjoinRoot infrastructure attempt would have been avoided.
**How to apply:** Before implementing any large polynomial identity in Lean: (1) express the identity with constituent sub-expressions as formal symbols; (2) substitute the defining relations one by one; (3) check whether the remainder is 0 (exact) or nonzero (needs quotient). The CAS test takes 5 minutes; the wrong architecture choice costs hours.

### [2026-06-25] Ask ChatGPT about DEFINITIONAL CONVENTIONS before proving — the zero-extension/junk-value trap
When a formal proof obligation involves a function defined with conditional/piecewise logic (zero extension outside a domain, junk values for undefined operations), dispatch a ChatGPT question specifically about how the proof system's CONVENTIONS interact with the mathematical claim BEFORE attempting a mathematical proof. The formal obligation may be trivially satisfiable (e.g. "derivative at endpoint = 0" because the junk-value convention returns 0 for non-differentiable functions) or fundamentally impossible (e.g. endpoint tendsto fails because zero extension creates a derivative discontinuity), and mathematical reasoning alone cannot distinguish these cases — only knowledge of the proof system's conventions can.
**Why:** A Neumann BC proof for a zero-extension function was expected to need 40+ lines of PDE-style argument. ChatGPT identified that the proof system's derivative convention (`deriv = 0` when not differentiable) makes it trivially true for ANY zero-extension function, and pointed to an existing lemma. Separately, ChatGPT correctly identified that a C²-to-tendsto approach would FAIL at endpoints due to the zero-extension discontinuity in the derivative — preventing a wasted attempt. Both insights came from understanding the formal system's conventions, not from mathematical content.
**How to apply:** When a proof obligation involves `deriv`, `fderiv`, or `HasDerivAt` at boundary points of a piecewise/zero-extension function, FIRST ask ChatGPT: "does the proof system's convention make this trivially true or impossible?" Check for existing lemmas about endpoint behavior of zero-extension functions. Do this BEFORE attempting a mathematical proof.

### [2026-06-25] Use ChatGPT as a MATHEMATICAL LITERATURE SEARCH tool — ask "is there a proof of theorem X that uses only tools Y?"
When a formalization is stuck because the known proof method requires infrastructure your framework lacks (e.g., modular forms when you work in formal power series), ask ChatGPT: "is there a published proof of [theorem] that uses ONLY [tools you have]? If so, cite the paper and outline the proof chain." ChatGPT can answer this question with specific author + year + arXiv ID + a concrete proof skeleton — far more targeted than keyword-searching arXiv, because it answers the MATHEMATICAL constraint ("proof using only bilateral summation, no modular forms") that a keyword search cannot express. The cited paper may use the exact tools you already have formalized.
**Why:** A formalization was stuck for 14 sessions because every known route required either modular forms (no formal bridge) or ring-closing identities with algebraically dependent atoms (ring tactic can't handle). Asking ChatGPT "is there a purely q-series proof using only JTP + bilateral summation?" yielded a specific 2012 paper by a named author, with a concrete 6-step proof chain — 3 of whose 6 steps were already proved in the repo. This single dispatch broke the 14-session logjam.
**How to apply:** When stuck on a deep theorem because the standard proof requires infrastructure X that your formal framework lacks, formulate: "is there a proof of [theorem name] that avoids X and uses only [tools you have]? Cite the paper." The answer either finds an alternative (actionable) or confirms none exists (you must build X). This is distinct from "route assessment" (which evaluates YOUR proposed routes) — this asks the collaborator to search its mathematical knowledge for routes you haven't considered.

### [2026-06-26] Graduate from ASSESSMENT → DETAIL → CODE across multi-round campaigns
For a complex multi-step proof task, structure the ChatGPT rounds as an escalating sequence: (1) ASSESS — "is there a proof using tools X? which route is shortest? what are the obstructions?" (2) DETAIL — "write out the exact equations/reindexing/exponents for route A" (3) CODE — "write compilable Lean code for theorem T using the infrastructure in file F." Each round's answer becomes the INPUT for the next. Do not jump to code-writing before the assessment confirms the route is viable and the detail pass nails the exact mathematical content. The graduated approach avoids writing code for the wrong route (wasted) and avoids design questions when you already know the route (slow).
**Why:** A 14-session-stuck problem was broken in one session using this graduated pattern: round 1 (assessment) found a new proof route via literature search; round 2-3 (detail) worked out the exact reindexing and exponent identities; rounds 4+ (code) produced compilable Lean theorems. Jumping straight to code would have targeted the wrong route; staying at assessment level would have delayed the code. The escalation was the right cadence.
**How to apply:** When starting a multi-round ChatGPT campaign on a complex proof: plan at least 3 rounds — assessment, detail, code — and don't skip ahead. Fire all channels at each level in parallel (different angles), act on convergence, then escalate. The assessment round is cheap (1 question per channel); the code round is expensive (long-think, may need iteration). Invest in the cheap rounds first.

### [2026-06-26] Use command-line argument for short questions, stdin heredoc for long — bare stdin pipe silently drops the question body
When dispatching via `ask-gpt.py`, bare `< /tmp/file.txt` stdin piping can silently fail to deliver the question body to the bridge — the channel receives only the channel-name label, and the answer comes back as a placeholder ("no question included"). The failure is silent: the script runs, blocks, and returns a committed git-drop with an empty/placeholder answer. Use command-line argument (`ask-gpt.py <channel> "question text"`) for questions under ~2000 chars, and `<<'EOQ' ... EOQ` heredoc for longer ones. Reserve `< file` piping only when the heredoc approach also fails.
**Why:** Three dispatches via `< /tmp/question.txt` produced empty placeholder answers (Q1003, Q1004) — ChatGPT received only the channel label. Switching to inline command-line argument delivered the same question successfully. The pipe failure was not flagged by the script.
**How to apply:** Default to inline argument for all dispatches. If the question is too long for the shell line, use heredoc. Only fall back to `< file` if both fail. On any placeholder/empty answer, suspect a delivery failure before re-asking — check whether the question body actually reached the tab.

### [2026-06-26] When auto-routing silently overrides your target channel, the DROP FILE gets clobbered — verify the Q# header matches YOUR dispatch
When multiple questions are dispatched to different channels but the bridge's auto-routing sends them to the SAME channel (e.g., cron3 → cron2), the later dispatch's answer overwrites the earlier one in the shared drop file. The git-drop commit reports success (a real SHA), but reading the drop file gives the WRONG answer — it contains the other question's response. Always verify the Q# header in the fetched drop file matches the Q# you dispatched before acting on the content. If it doesn't match, the answer was clobbered — re-dispatch to a confirmed-working channel.
**Why:** Questions dispatched to cron1 and cron3 were both auto-routed to cron2's drop file; the later answer overwrote the earlier one, and the earlier answer was lost. The git-drop commit succeeded for both, but only one answer survived in the file. The mismatch was only caught by reading the Q# header.
**How to apply:** After every git-drop harvest, grep the first line for `Q<N>` and confirm it matches your dispatched Q#. If it shows a different Q#, your answer was clobbered — check git log for the expected commit, or re-dispatch. Pairs with the freshness-check entry (which checks staleness from prior rounds; this checks clobbering from parallel routes).

### [2026-06-27] For a structural claim about a theorem's interface, ask the collaborator to READ the type signature — quantifier-level questions are definitively answerable from source
When you have a structural claim about a theorem's quantifiers (∀w vs per-w, ∃T vs uniform T, uniform bound vs per-input bound), dispatch a focused question asking the collaborator to READ THE LEAN TYPE SIGNATURE and verify. This is the highest-confidence use of a code-reading collaborator: type-level structure is objective, readable in one file, and definitively answers the structural question. Compare with open-ended "find an escape" questions (low yield) or mathematical feasibility questions (needs real analysis, error-prone). The type signature IS the ground truth.
**Why:** Three breakthroughs in one session came from type-verification dispatches: "does EventualThresholdSimulation.correct_halt have ∃T or ∀T?" (confirmed per-w), "does bounded require uniform-in-w?" (confirmed stereo ≤ 1, free), "does hMnext at haltCoord carry mult*ρu?" (confirmed zero by type). Each resolved a multi-hour route-analysis question in one dispatch.
**How to apply:** When your route analysis hinges on a structural claim about a Lean theorem's interface (quantifier order, field presence, parameter dependence), don't reason about it — dispatch "read file X, line Y, verify claim Z" to the collaborator. The answer is definitive and fast. Pair with #119 ("is my framing correct?") for the broader framing question, but use THIS for the narrow type-level verification.

### [2026-06-27] Ask the collaborator to CHAIN EXISTING INFRASTRUCTURE, not just write new code — the repo's own building blocks may close the gap
When you're stuck on a sorry/gap, the highest-value question is often NOT "write new proof code for X" but "what EXISTING theorems in the repo can be chained to close X?" A repo-connected collaborator can read type signatures across files, trace dependency chains, and discover that the gap is already closable by composing 2-3 existing theorems you didn't know about. This is fundamentally different from asking for new code — the collaborator acts as an infrastructure MAP, revealing existing building blocks and their composition order. The answer is a short chain (`A → B → C → done`) with the exact theorem names and how they compose.
**Why:** Asked "how to prove srcTimeCoeff is ContDiffAt ℝ 2 at positive time" and the answer revealed an existing theorem (`srcTimeCoeff_contDiffAt`) with all three sub-lemmas already proved — and an existing data package (`FlooredSourceTimeData`) that instantiates it for the specific case. The gap that looked like "deep new analytic content" was actually a 6-line composition of existing infrastructure. Hours of planned new-infrastructure work became unnecessary.
**How to apply:** Before writing new infrastructure for a gap, dispatch: "read [the files around the gap], and tell me: is there an existing theorem chain in the repo that already closes or reduces this gap? Name each theorem and how they compose." The collaborator's repo-read + reasoning finds chains that `grep` misses (because the chain involves type-level composition, not textual matches). Reserve "write new code" dispatches for when the infrastructure-chain question comes back empty.

### [2026-06-27] Channel routing is random by design — always check ALL drop files for your answer
When dispatching to a named channel (e.g. `family3`), the answer may land in a DIFFERENT channel's drop file (e.g. `family1`). This is by design: the bridge routes questions to whichever tab is idle, not necessarily the one you named. When harvesting git-drops, always check ALL drop files (`_CHATGPT_DROP_family1.md`, `_CHATGPT_DROP_family2.md`, etc.), not just the one matching the channel you dispatched to. If the expected channel shows stale content but another channel has a fresh commit with your question's topic, that IS your answer.
**Why:** Dispatched a negative-binomial proof question to `family3`, but the answer appeared in `_CHATGPT_DROP_family1.md` while `family3` kept showing an old Q1000 audit. Wasted time re-dispatching before Xiang confirmed: "这个是 feature, 我本来就是怎么设计的" — the routing is intentionally random across idle tabs.
**How to apply:** On every git-drop harvest: `git fetch origin chatgpt-drop && for f in family1 family2 family3; do echo "--- $f ---" && git show origin/chatgpt-drop:scratch/_CHATGPT_DROP_${f}.md | head -5; done`. Match by topic/Q-number, not by channel name. Never assume channel X's answer is in channel X's file.
