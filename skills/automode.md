...
Don't wait until your context is exhausted (see GRINDER→ORCHESTRATOR) and never rely on the USER to manually hand you off or restart you — that is a dumb failure the user explicitly flagged. The moment you catch yourself about to DEFER a laborious/deep piece ("needs fresh context", "I'll be error-prone here") or about to ask the user to restart you, that is the signal to spawn a fresh-context SUB-AGENT (your own harness's agent primitive — same capability, clean context, full tools incl. build/dispatch) and orchestrate it. Sub-agents are not only for exhaustion; they are your STANDING, proactive context-management tool, and you can run SEVERAL in parallel on independent hard pieces (plus fire an independent external reasoner alongside).
**Why:** I repeatedly invoked "context too deep / needs fresh context" as a reason to defer, or leaned on the user to hand off and restart me; the user's correction was pointed — don't you have a way to proactively manage your own context instead of making me do the handoff? The mechanism existed (spawn parallel fresh-context sub-agents) — I just wasn't reaching for it proactively.
**How to apply:** Treat "I'm about to defer for fresh context" or "the user should restart me" as a fingerprint → instead spawn fresh-context sub-agent(s) with a self-contained brief and become the verify/wire/commit orchestrator; parallelize across independent hard pieces. Reserve the user only for genuine hard-stops (external resource, destructive/external action, goal complete, all avenues exhausted). Extends GRINDER→ORCHESTRATOR (exhaustion-triggered, external-LLM) to the PROACTIVE, self-managed, sub-agent-and-parallel case.
...
A conditional result `(h : P) … : C` compiles green and passes every foundation check while `P` is UNSATISFIABLE — the build never tries to INHABIT `P`, it only assumes it. So "it builds / it's clean / the wiring landed end-to-end" is NOT the completion bar for any milestone that CARRIES hypotheses. Reporting such a milestone "complete / done / bottomed-out" invites a costly public walk-back the moment someone tries to instantiate the carried surface and finds it empty. This is the precise gap left by "don't report nearly-done until it compiles green": for a CONDITIONAL result, compiling green is itself insufficient.
**Why:** In an autonomous run a milestone was reported "complete" on landed wiring + a green, foundation-clean build; a later attempt to instantiate ONE carried surface found it unsatisfiable (its predicate excluded only the extreme corner of the bad set, not the interior), forcing a correction to the owner. The green build had silently carried the impossible hypothesis — twice in one run.
**How to apply:** Before reporting any conditional milestone "complete", for EACH carried hypothesis either exhibit a concrete inhabitant/witness or prove the surface is inhabited — not merely that it has the right shape. If you cannot witness a carried surface, report it as a NAMED carried residual, never as "done". Green-compile + foundation-clean is necessary, not sufficient; satisfiability-witnessing of carried objects is the separate, required gate. Pairs with "run the end-to-end assembly early" and "report only verified-stable state".
...
When the autonomous goal is to discharge a multi-FIELD obligation (a record/structure/bundle with N fields, each closable by a different already-built producer), do NOT write the whole closed assembler in one giant pass, and do NOT just take all N fields as hypotheses (that re-wrapper is a NO-OP). Wire it INCREMENTALLY: discharge ONE field from its producer (which surfaces that field's concrete lower-level inputs as new hypotheses), carry the remaining N−1 fields as explicit hypotheses, compile + commit GREEN, then wire the next field. Each wire trades one opaque field-obligation for the concrete constraints its producer needs — so the live hypothesis list of the partial assembler IS your remaining-work board, shrinking and sharpening by one with every green commit. You never hold a giant uncommitted multi-field change, and the constraints reveal themselves in dependency order instead of being guessed up front.
**Why:** Driving a many-field assembly obligation, I wired the fields one at a time from banked producers — each commit was green + axiom-clean and its signature exposed exactly the next layer's concrete inequalities; the remaining-as-hypotheses fields made the partial assembler a precise, compilable statement of "what's left", and the dependency order (which atom feeds which) emerged from the wiring rather than from speculation. A single monolithic pass would have been an error-prone uncommitted blob that hid the constraint structure.
**How to apply:** Autonomous discharge of an N-field bundle from N producers → wire field-by-field: one producer per commit, rest carried as hypotheses, green + committed each step; read the growing hypothesis list as the live atom board. Specializes assemble-early (which says DO the end-to-end assembly to find the keystone) and trunk-green-incremental (additive layers for a refactor) to the multi-field-RECORD case, where each field-wire is both a discharge AND a constraint-reveal. The re-wrapper (take all fields as hyps) is a NO-OP; the value is in replacing each field-hyp with its producer call.
...
When a sub-obligation is framed as "prove this event has small probability" and multiple attack vectors fail (the tail isn't small, the existing machinery doesn't yield a bound, the right potential doesn't exist), the bottleneck may not be proof difficulty but a wrong CLASSIFICATION: the event is not rare — it is the legitimate system behavior, and the obligation should be met by a different object entirely (e.g. a structural window invariant, not a concentration tail). Re-classifying the residual's nature (tail → window, concentration → deterministic invariant, avoidance → timing separation) dissolves the grind. This is NOT 退堂鼓 or path-switching — it is correcting the problem statement while keeping the goal fixed.
**Why:** A residual was framed as a first-passage concentration tail to prove exponentially small. Three attack vectors failed; an independent design question revealed the event was the system's INTENDED transition gate (not a failure), making the tail-framing a category error. Re-classifying it as a convergence-window-surface problem dissolved the grind into a structural insight that closed trivially. The same pattern appeared at three sub-problems in one run, each initially framed as concentration but actually needing a window/invariant.
**How to apply:** When a sub-obligation resists ≥2 concentration attack vectors, before trying a third: ask "is this event actually RARE, or is it the legitimate/designed behavior?" (dispatch this as a design question to an independent reasoner). If the event is legitimate, re-classify the residual from "tail" to "window/invariant/timing" and re-dispatch. Grinding harder within a wrong classification is the same waste as grinding a false axiom — recognizing the category error is the productive move. Pairs with the PIVOT entry (which covers architecture-level re-routing) and the conditional-milestone-witnessing entry (which catches unsatisfiable surfaces).
...
In a multi-atom discharge campaign, a subset of "open" atoms may NOT be independently closable from schedule algebra or banked producers: they are SOLUTION-DEPENDENT facts (the solution's analytic behavior at specific times) that the equivalent parallel route also CARRIES as explicit hypotheses in its own conditional headline. Grinding them as standalone lemmas is wrong — they should be CARRIED by your conditional headline as its interface, discharged only by the next-layer campaign (tracking convergence / solution-existence). The signal: if the parallel route's capstone carries the same-shaped hypothesis, your atom is a carried premise, not a leaf to close now.
**Why:** After wiring 7/9 fields of a multi-field assembler, the remaining atoms (initial-weighted, hold-slack, z-read tubes, u-copy, z-sup, flag) resisted standalone discharge; reading the parallel route's capstone revealed it carries exactly those atoms as explicit hypotheses — they are solution-dependent "Reach" premises, not schedule algebra. Reclassifying them from "open atoms to grind" to "carried premises matching the parallel arch" corrected the campaign's structure and created a clean architectural boundary (conditional headline → tracking-convergence campaign).
**How to apply:** When a subset of open atoms in a multi-field discharge resists standalone closure, read the PARALLEL ROUTE's capstone for the same goal. If it carries the same-shaped hypotheses, your atoms are carried premises — stop grinding them independently and carry them in your conditional headline. The next campaign (tracking convergence) discharges them. Distinct from "N instances never close ∀" (generality mismatch) and "assemble early" (finding the keystone) — this is about CLASSIFICATION of the remaining atoms.
...
For a task that needs both architecture validation and code production, dispatch BOTH in parallel: a design/feasibility question to the reasoning collaborator, and the implementation attempt to a fresh-context sub-agent. The sub-agent tries to build the code; the collaborator analyzes whether the approach is sound. When both return, you cross-validate: if the sub-agent's code compiles and the collaborator's design agrees on the key decisions (carried hypotheses, extra inputs, case-split structure), the result is doubly-verified. If they disagree, the disagreement points you to the exact gap. This is strictly faster than serial (design first, then implement) and more reliable than implementation-only (you might build the wrong thing).
**Why:** A shifted-Euclidean assembly was implemented by a sub-agent (read files + write code + build) while a parallel collaborator question analyzed the readout architecture. Both independently converged on the same design decision (an extra low-flag hypothesis for early cycles). The sub-agent's file compiled clean-3; the collaborator's analysis confirmed the architecture was sound — doubly-verified in the time of the slower one, instead of serially.
**How to apply:** When a new piece of the proof chain needs both "is this the right approach?" and "write the code": fire both simultaneously — design question to the reasoning collaborator, implementation to a sub-agent. Cross-validate on return. Distinct from dispatch-as-race (both doing the SAME work) — this is complementary-split (different aspects of the SAME task in parallel).

### [2026-06-25] Prove sorry-free theorems one at a time — don't build sorry-laden skeletons
In an autonomous formalization grind, the most productive unit of work is a SINGLE sorry-free theorem that compiles and is permanently banked. The tempting alternative — writing a multi-sorry skeleton first, then filling sorry one by one — is strictly worse because: (1) each sorry becomes a future build-fix cycle with namespace errors, type mismatches, and API wiring issues; (2) downstream sorry can't compile until upstream sorry are filled, so the skeleton doesn't verify the architecture; (3) the sorry count becomes a misleading progress metric (going from 18 to 9 "feels like progress" but none of those sorry are discharged). Instead: identify the leaf-level lemma, prove it sorry-free, verify it builds, then move to the next leaf. A session that produces 6 sorry-free theorems + 9 remaining sorry achieved less verified progress than one that produces 6 sorry-free theorems + 0 sorry.
**Why:** A session created 3 skeleton files with 18 sorry to outline an iterate tower + limit passage architecture. The session then spent its entire runtime chipping sorry from 18→9 through build-fix cycles (namespace paths, theorem→def, set_option positioning, type mismatches). The sorry-free theorems (composition chain rule, endpoint derivative convention, definitional bridge) were each proved cleanly in one shot. The skeleton's sorry served as a roadmap but not as verified architecture — each filled sorry revealed new type-level issues that a direct sorry-free approach would have caught immediately.
**How to apply:** When starting a new proof chain in automode: (1) identify the leaf lemma in the dependency DAG; (2) prove it sorry-free with a single file, verify build; (3) move to the next leaf. Don't write the full multi-file skeleton first. If you need to record the architecture, write it in DOCTRINE.md (prose), not in sorry'd Lean files.

### [2026-06-26] "Preparation work that doesn't produce executable code" is a disguised wait — the measure of ready is independent work you CAN complete, not handoff plans
When a run has multiple parallel avenues with external blockers (collaborator working on X, ChatGPT thinking on Y), the temptation is to declare "framework is ready, preparing to integrate" and list the integration steps. This is a fingerprint of 退堂鼓 wearing a "readiness" disguise. The hard boundary: if you can complete MORE INDEPENDENT WORK on the current avenue before external blockers clear (a sibling lemma, the mechanical wiring of an existing scaffold, a different piece of the same proof), then "readiness" is not a stopping point — it is a false milestone. The true measure of ready is: "I have zero more work I can do without external input" (not "my integration plan is very solid").
**Why:** In an automode session, I went from "three axiom lines are frameworks-complete and ready to integrate" to "waiting for external feedback." Midway through, I realized I could (and SHOULD) continue: filling mechanical parts of the locally-complete framework (C-Line), finishing case-split structures (A-Line). The "framework ready" was real, but it was not a checkpoint — it was a handoff template that could wait. What mattered was whether I had exhausted all independent work. I had not.
**How to apply:** When you find yourself about to report "X is ready to integrate, awaiting external," STOP and ask: "Do I have any independent work LEFT on X, or on other avenues that does NOT depend on the external result?" If yes, grind that first, then report. "Ready" means "I have done every scrap of independent work possible;" framework-structure alone is not ready. Pairs with anti-pattern #6 (避免"到边界"framing) and the "dispatch is a race, not a block" mindset: external work frees your mental cycles, it does not pause the ground truth of what work remains.

### [2026-06-27] Don't declare "genuine frontier / irreducible gap" until you've checked the PARAMETER SPACE and QUANTIFIER STRUCTURE — a structural-sounding wall may dissolve
...
### [2026-06-27] For large-codebase proof grinding, dispatch Codex FIRST — Claude subagents stall on file-reading
When the task requires reading many files in a large codebase before writing code (e.g., a Lean formalization with 100+ files, a multi-module refactor), dispatch it to the external coding agent (Codex/GPT-5.5) rather than spawning Claude subagents. Claude subagents spend their entire context window reading files and stall before producing any code — they hit context limits mid-analysis. The external agent has its own independent context, reads files via shell tools at its own pace, and reliably produces hundreds of lines of working infrastructure. Reserve Claude subagents for tasks that need ≤3 files of context.
**Why:** Three Claude subagents were dispatched on the same proof task over ~2 hours. Each consumed 400-500KB of context reading files, produced zero lines of code, and stalled or was killed. One Codex (GPT-5.5) session read the same files and wrote 300 lines of compilable infrastructure in 30 minutes. The difference: Codex manages its own context and writes incrementally; Claude subagents try to load everything into memory first and run out of room.
**How to apply:** When the task involves a large codebase (>10 files to read, >1000 lines of context needed): dispatch to Codex (`codex -m gpt-5.5 -a never "task"`), NOT to a Claude Agent subagent. Use Claude subagents only for small, focused tasks (single-file edits, targeted searches, ≤3 files). If you catch yourself spawning a SECOND Claude subagent on the same large-codebase task after the first stalled — stop, kill it, and dispatch Codex instead.

### [2026-06-27] Verify inherited DOCTRINE's MATHEMATICAL claims before committing to an avenue
When a cross-session handoff DOCTRINE contains mathematical/technical claims that an avenue's viability depends on (e.g., "a support condition follows from quadratic growth", "the recurrence has a unique solution", "the cancellation is guaranteed by periodicity"), treat each claim as a HYPOTHESIS — not a fact — until independently verified. Dispatch a verification question to a collaborator (or write 10 lines of computation) BEFORE building infrastructure on the claim. An unverified claim that turns out false wastes the entire infrastructure built on it.
**Why:** A prior session's DOCTRINE claimed a support condition "follows from quadratic q-growth of factors." The current session built ~300 lines of sigma-transport infrastructure on this claim. A concrete computation (confirmed by an independent collaborator) showed the claim is FALSE — the composition has a genuine bad ray that no factor-level argument can remove. The entire infrastructure was wasted. 10 lines of Python or one ChatGPT question would have caught this before any infrastructure was written.
**How to apply:** At automode start, after reading the inherited DOCTRINE: for each avenue whose viability depends on a mathematical/technical assertion, ask "has this assertion been PROVED or is it a conjecture?" If conjecture: dispatch verification (Python computation or collaborator question) as the FIRST action, before writing any Lean/code. An avenue built on a false claim is worse than no avenue — it sinks hours before the falsity is discovered. Pairs with "run the end-to-end assembly EARLY" (which catches structural issues) — this catches PREMISE issues.

### [2026-06-27] Polling a background process's log IS idle work — do independent work, let the notification fire
